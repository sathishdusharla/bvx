...
const Results = () => {
  ...
  const [electionName, setElectionName] = useState('Election Name'); // Default election name
  ...
  useEffect(() => {
    const fetchElectionData = async () => {
      try {
        ...
        setElectionName(data.electionName || 'Election Name'); // Set the election name from the response
        setLoading(false);
      } catch (error) {
  ...
  const downloadResults = () => {
    ...
    const totalVotes = results.reduce((acc, curr) => acc + curr.votes, 0);
    doc.setFontSize(12);
    doc.text(`Total Votes: ${totalVotes}`, 105, (doc as any).lastAutoTable.finalY + 20, { align: 'center' });
    ...
    <div className="min-h-screen pt-20 pb-12 bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-white mb-4">{electionName}</h1>
